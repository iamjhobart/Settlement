CREATE PROCEDURE [Payments].[usp_Payment_CloseFBInAlltheFPSPaymentEnvironmentsforVisibility]
AS
...
BEGIN
	...
	BEGIN TRY
	  ...
	  SELECT @Query = 
	  ' 
	  [' + @DNS + '].[' + @DB + '].[Payments].[usp_Payment_CloseFB] ''' + CAST(@EnvironmentId AS NVARCHAR(100)) + ''', ''' + CAST(@BusinessFlow AS NVARCHAR(100)) + ''', ''' + CAST(@ExecPath AS NVARCHAR(100)) + ''', ' + CAST(@CompleteInvoiceOnly AS VARCHAR(1)) + ', ' + CAST(@GenerateRemitAdv AS VARCHAR(1)) + ', ' + CAST(@NoOfDaysPrior AS VARCHAR(1)) +'
	  '
