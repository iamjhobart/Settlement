...
SET @EnvironmentId 			= 'A2736AF2-701B-4718-93E2-1545DCE64C71'
...
BEGIN      
	...
	SELECT         
		...
		CASE WHEN @CheckDateToUse = 'InvoiceDueDate' THEN DATEADD(dd, DATEDIFF(dd, 0, CAST(ISNUll(dbo.INVOICE.INV_DUE_DTM,GetDate()) as datetime)), 0)
			 WHEN @CheckDateToUse = 'InvoiceCreateDate' THEN DATEADD(dd, DATEDIFF(dd, 0, CAST(ISNUll(dbo.INVOICE.INV_CREAT_DTM,GetDate()) as datetime)), 0)
			 WHEN @CheckDateToUse = 'ForwardDate' THEN DATEADD(dd, DATEDIFF(dd, 0, CAST(ISNUll(dbo.PAYR_DTL.[%T003],GetDate()) as datetime)), 0)
			 WHEN @CheckDateToUse = 'ExecutionDate' THEN DATEADD(dd, DATEDIFF(dd, 0, GetDate()), 0)
			 WHEN @CheckDateToUse = 'InPaymentNew' THEN DATEADD(dd, DATEDIFF(dd, 0, CAST(ISNUll(dbo.PAYR_DTL.RCRD_CREAT_DTM,GetDate()) as datetime)), 0)
		END AS PaymentDate
