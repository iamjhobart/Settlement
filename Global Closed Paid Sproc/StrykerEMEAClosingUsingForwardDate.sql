...
DECLARE @CompletedInvoiceOnly  	AS Bit      
DECLARE @GenerateRemitedAdvises AS Bit 
DECLARE @NoOfDaysPrior 			AS Int 
DECLARE @CheckDateToUse			AS VARCHAR(50) 
      
SET @EnvironmentId 			= 'DEAEB5F2-7350-4E2A-B48A-B7B44779262A'
SET @BusinessFlow  			= NULL
SET @ExecPath	   			= NULL
SET @CompletedInvoiceOnly  	= 1      
SET @GenerateRemitedAdvises = 0  
SET @NoOfDaysPrior 		    = 0 
SET @CheckDateToUse			= 'ExecutionDate' --InvoiceDueDate, ForwardDate, ExecutionDate
...
BEGIN
	...
	SELECT         
		...
		CASE WHEN @CheckDateToUse = 'InvoiceDueDate' THEN DATEADD(dd, DATEDIFF(dd, 0, CAST(ISNUll(dbo.outINVOICE.INV_DUE_DTM,GetDate()) as datetime)), 0)
			 ...
			 WHEN @CheckDateToUse = 'Manual' THEN DATEADD(dd, DATEDIFF(dd, 0, CAST(ISNUll(dbo.PAYR_DTL.RCRD_CREAT_DTM,GetDate()) as datetime)), 0)
		END AS PaymentDate
	...
	WHERE    
	    ...
	    --AND PAYR_DTL.fwrd_key in ('300035')
...
BEGIN      
	...
	SELECT         
		...
		FRGHT_BL.FB_APP_AMT,      
		ISNULL(PAYR_DTL.PAYMT_REQ_CURRENCY_QUAL,'USD') AS CURRENCYCODE,
		CASE WHEN @CheckDateToUse = 'InvoiceDueDate' THEN DATEADD(dd, DATEDIFF(dd, 0, CAST(ISNUll(dbo.INVOICE.INV_DUE_DTM,GetDate()) as datetime)), 0)
			 WHEN @CheckDateToUse = 'ForwardDate' THEN DATEADD(dd, DATEDIFF(dd, 0, CAST(ISNUll(dbo.PAYR_DTL.[%T003],GetDate()) as datetime)), 0)
			 WHEN @CheckDateToUse = 'ExecutionDate' THEN DATEADD(dd, DATEDIFF(dd, 0, GetDate()), 0)
			 WHEN @CheckDateToUse = 'Manual' THEN DATEADD(dd, DATEDIFF(dd, 0, CAST(ISNUll(dbo.PAYR_DTL.RCRD_CREAT_DTM,GetDate()) as datetime)), 0)
		END AS PaymentDate
	...
	INNER JOIN  dbo.INVOICE WITH (NOLOCK)      
 		ON PAYR_DTL.INV_ID=INVOICE.INV_ID

    INNER JOIN dbo.INVOICE_EXT WITH (NOLOCK)
    	ON INVOICE.INV_ID=INVOICE_EXT.INV_ID
	...
	WHERE
		dbo.FRGHT_BL.FB_ID LIKE 'FBLL' + @FixOwnerKey + '%'     
 		...
 		AND PAYR_DTL.RCRD_CREAT_DTM<=DATEADD(dd, @NoOfDaysPrior, GETDATE())      
 		AND PAYR_DTL.FUNDS_APP_TEMP_ID IS NULL      
	    --AND FRGHT_BL.INV_ID = 'INVC0060462000173600001'
...
--select p.fwrd_key,p.FUNDS_REQ_KEY,flow.BusinessFlow,flow.ExecPath, f.*
--into Payments.JennyTemp
--	FROM #t_FBInfo  AS F
...
--select * from Payments.JennyTemp 
...
SELECT  Row_Number() OVER (ORDER BY OWNER_KEY,  VEND_LABL, CURRENCYCODE, PAYMT_TYPE, [PaymentDate] ) as PAYMENT_ID,      
  OWNER_KEY,      
  VEND_LABL,       
...
SELECT   Header.PAYMENT_ID,      
  ...
  Detail.FB_APP_AMT,      
  Header.CURRENCYCODE,
  Detail.PaymentDate as [CheckDate]        
...
INNER JOIN  #t_PaymentHeader AS Header       
 ...
 AND Header.CheckDate =Detail.PaymentDate 
...
INNER JOIN #t_FUNDS_AP  ON  #t_PaymentDetail.CURRENCYCODE=#t_FUNDS_AP.FUNDS_APP_CURRENCY_QUAL      
     ...
     AND  #t_PaymentDetail.CheckDate=#t_FUNDS_AP.CREAT_DTM
           
--select * from #t_PAYR_DTL_Update 
--select * from #t_PaymentDetail     
...
select owner_name, a.inv_id, i.inv_key, inv_stat, inv_fb_cnt, inv_amt, inv_app_amt, inv_adjm_amt, PaymentDate, sum(paymt_req_amt) as paymt_req_amt from #t_FBInfo a
inner join invoice i (nolock) on a.inv_id = i.inv_id
inner join payr_dtl p (nolock) on i.inv_id = p.inv_id
inner join owners o (nolock) on i.owner_key = o.owner_key
group by owner_name, a.inv_id, i.inv_key, inv_stat, inv_fb_cnt, inv_amt, inv_app_amt, inv_adjm_amt, PaymentDate

select * from #t_FUNDS_AP